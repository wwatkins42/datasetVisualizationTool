...
# http://bids.github.io/colormap/ for viridis/plasma/magma/inferno colormaps
cmaps = {
    'dark':['#000000', '#303030', '#202020', '#404040'],
...
def makeBoxPlots(data, labels, features_type, axis=2, normed=False):
    ...
    for j in range(data.shape[1]):
        if features_type[j] == 1:
            ...
            if normed is True:
                cmin = np.min(col)
                cmax = np.max(col)
                col = (col - cmax) / (cmax - cmin) + 1. # TODO: raises RuntimeWarning "invalid value encountered in divide"
        else:
            col = [0]
        dicts.append(
            go.Box(
...
# def makeVAlignedAnnotations(texts, x, y, d, xref='paper', yref='paper', xanchor='left', reversed=True):
#     if reversed is True:
#         d = -d
#     dicts = []
